...
## Why I chose this:
I got into computer science in high school because I wanted to make video games. The method for testing video games is to just have peopel play them however they want. A large part of this class has been related to automated testing, so I wanted to see if there where any tools for automated testing in video game engines. Unity and Unreal engine are the two most popular game development engines, so I'll be going over each of their automated testing frameworks. 


## Unity Test Framework:
Unitys Test framework has two kinds of tests: Edit Mode and Play Mode. 
...
- Play Mode lets you run tests on the game itself while it's running, like an automated playtest. This mode is used for gameplay tests, integration tests, and performance tests. 
...
4. Create your test scripts in that test folder and write the tests in C#. 
...
7. Use assert commands to check results. 

Unity also has external libraries like NSubstitute. This is simialr to Jest and allows mocking objects or classes for your tests. 

## Unreal Automation Test Framework: 
Tests in Unreal can be written using normal C++ code, or using blueprints. Blueprints represent code as nodes that represent a functions and can be connected to perform different functions, similar to code blocks in Scratch. 
...
## Conclusion:
Before making this report I only knew about manual game testing. I did know about tool assisted speedruns where players create a bot to beat a game with theoretically perfect execution. The idea of automating a test for a game isn't that far off. There where many more types of automated tests than I thoght there where. These tools will definitly be useful when I eventually try making my own videogame.